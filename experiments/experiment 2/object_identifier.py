...
from delta_interfaces.msg import DetectedObjects
...
class LevelObjectIdentifier(Node):
    ...
    def __init__(self):
        ...
        # subscriber to receive objects from detected_people
        self.object_subscription = self.create_subscription(DetectedObjects, "/some_detected_object", self.receive_marker, 1)
        self.object_subscription  # prevent unused variable warning
    ...
    def receive_marker(self, msg):
        robot_frame_x = msg.position_x
        robot_frame_y = msg.position_y
        robot_frame_z = msg.position_z
        park_x = msg.park_position_x
        park_y = msg.park_position_y
        park_z = msg.park_position_z
        
        
        # transforming from robot frame to map frame
        face_map_position = self.transform_from_robot_to_map_frame(robot_frame_x, robot_frame_y, robot_frame_z)
        park_map_position = self.transform_from_robot_to_map_frame(park_x, park_y, park_z)
        if (face_map_position is not None) and (park_map_position is not None):
            self.process_face_position_in_map_frame(face_map_position[0], face_map_position[1] ,face_map_position[2], park_map_position[0], park_map_position[1], park_map_position[2])


    # processes the position of a face that was spottet in the map coordinate frame at face_x, face_y, face_z
    def process_face_position_in_map_frame(self, face_x, face_y ,face_z, park_x, park_y, park_z):
        ...
        rotation = 0.0
        self.insert_level_object(face_x, face_y, face_z, rotation,"person_"+str(self.personId))
