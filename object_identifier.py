...
class LevelObjectIdentifier(Node):
    ...
    def process_face_position_in_map_frame(self, face_x, face_y ,face_z, robot_frame_y):
        ...
        robot_to_face_vector = -self.unit_vector(robot_to_face_vector) * 0.3


        p_x = face_x + robot_to_face_vector[0]
        p_y = face_y + robot_to_face_vector[1]
        p_z = 1.0

        robot_forward = self.transform_from_robot_to_map_frame_safe(1.0, 0.0)
        angle = self.angle_between(robot_forward[0], robot_forward[1], robot_to_face_vector[0], 1)
        if (robot_frame_y > 0):
            angle = - angle
        rotation = angle
